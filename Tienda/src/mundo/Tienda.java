...
public class Tienda {
    ...
    public double darPromedioVentas() {
        ...
        int totalProductosConVentas = 0;
        // En los if traemos el valor de las unidades vendidas, si es mayor a 0
        totalVentas += producto1.darCantidadUnidadesVendidas();
        if (producto1.darCantidadUnidadesVendidas() > 0) {
            totalProductosConVentas++;
        }
        totalVentas += producto2.darCantidadUnidadesVendidas();
        if (producto2.darCantidadUnidadesVendidas() > 0) {
            totalProductosConVentas++;
        }
        totalVentas += producto3.darCantidadUnidadesVendidas();
        if (producto3.darCantidadUnidadesVendidas() > 0) {
            totalProductosConVentas++;
        }
        totalVentas += producto4.darCantidadUnidadesVendidas();
        if (producto4.darCantidadUnidadesVendidas() > 0) {
            totalProductosConVentas++;
        }
        if (totalProductosConVentas > 0) { // Si es mayor a 0, sacos el promedio
            respuesta = totalVentas / totalProductosConVentas;
        }
    ...
    public Producto darProductoMasVendido() {
        ...
        // Verficamos si la cantidad de unidades vendidas de un producto especifico es mayor que las cantidades de unidades vendidas de los otros tres productos
        if (producto1.darCantidadUnidadesVendidas() > producto2.darCantidadUnidadesVendidas() &&
                producto1.darCantidadUnidadesVendidas() > producto3.darCantidadUnidadesVendidas() &&
                producto1.darCantidadUnidadesVendidas() > producto4.darCantidadUnidadesVendidas()) {
            masVendido = producto1;
        } else if (producto2.darCantidadUnidadesVendidas() > producto1.darCantidadUnidadesVendidas() &&
                producto2.darCantidadUnidadesVendidas() > producto3.darCantidadUnidadesVendidas() &&
                producto2.darCantidadUnidadesVendidas() > producto4.darCantidadUnidadesVendidas()) {
            masVendido = producto2;
        } else if (producto3.darCantidadUnidadesVendidas() > producto1.darCantidadUnidadesVendidas() &&
                producto3.darCantidadUnidadesVendidas() > producto2.darCantidadUnidadesVendidas() &&
                producto3.darCantidadUnidadesVendidas() > producto4.darCantidadUnidadesVendidas()) {
            masVendido = producto3;
        } else if (producto4.darCantidadUnidadesVendidas() > producto1.darCantidadUnidadesVendidas() &&
                producto4.darCantidadUnidadesVendidas() > producto2.darCantidadUnidadesVendidas() &&
                producto4.darCantidadUnidadesVendidas() > producto3.darCantidadUnidadesVendidas()) {
    ...
    public Producto darProductoMenosVendido() {
        ...
        // Con los if validamos que sea mayor a 0
        if (producto1.darCantidadUnidadesVendidas() > 0) {
            menosVendido = producto1;
        }
        // En los siguientes if se valida si esa cantidad es menor que la cantidad de unidades vendidas del producto actualmente almacenado en la variable
        if (producto2.darCantidadUnidadesVendidas() > 0 &&
                (menosVendido == null || producto2.darCantidadUnidadesVendidas() < menosVendido.darCantidadUnidadesVendidas())) {
            menosVendido = producto2;
        }
        if (producto3.darCantidadUnidadesVendidas() > 0 &&
                (menosVendido == null || producto3.darCantidadUnidadesVendidas() < menosVendido.darCantidadUnidadesVendidas())) {
            menosVendido = producto3;
        }
        if (producto4.darCantidadUnidadesVendidas() > 0 &&
                (menosVendido == null || producto4.darCantidadUnidadesVendidas() < menosVendido.darCantidadUnidadesVendidas())) {
    ...
    public int venderProducto(String pNombreProducto, int pCantidad) {
        ...
        if (producto != null) {
            producto.cambiardarCantidadUnidadesVendidas(cantidadVendida); // Actualiza la cantidad de unidades vendidas
            if (pCantidad > producto.darCantidadBodega()) { // Verificar si la cantidad a vender es mayor que la cantidad en bodega
                cantidadVendida = producto.darCantidadBodega(); // Vender solo lo disponible en bodega
            } else {
                cantidadVendida = pCantidad;
            }
            double precioUnitarioConIVA = producto.calcularPrecioFinal();
            double totalVenta = cantidadVendida * precioUnitarioConIVA;
            dineroEnCaja += totalVenta;

            // Realiza la venta ajustando la cantidad en bodega
    ...
    public boolean abastecerProducto(String pNombreProducto, int pCantidad) {
        ...
        if (producto != null && producto.puedeAbastecer()) { // Validamos si
    ...
    public boolean cambiarProducto(String pNombreActual, String pNombreNuevo, String pTipo, double pValorUnitario, int pCantidadBodega, int pCantidadMinima, String pRutaImagen) {
        ...
        if (productoActual != null && (productoNuevo == null || productoNuevo == productoActual)) {
            if (productoActual.puedeAbastecer()) { // Verificar si puede abastecer antes de realizar el cambio
                ...
                productoActual.cambiarRutaImagen(pRutaImagen); // se crea el metodo cambiarRutaImagen()
            }
        }
        return cambio;
    }
    ...
    public int metodo1() {
        // TODO: "Completar"
        double promedioPrecios = (producto1.darValorUnitario() + producto2.darValorUnitario() +
                producto3.darValorUnitario() + producto4.darValorUnitario()) / 4;
        // Valores menores al promedio de precios
        int contador = 0;
    ...
    public String metodo2() {
        ...
        // Empezamos suponiendo que el producto1 es el mas barato, y luego validamos los siguientes
        if (producto2.darValorUnitario() < productoMasBarato.darValorUnitario()) {
            productoMasBarato = producto2;
        }
        if (producto3.darValorUnitario() < productoMasBarato.darValorUnitario()) {
            productoMasBarato = producto3;
        }
        if (producto4.darValorUnitario() < productoMasBarato.darValorUnitario()) {
            productoMasBarato = producto4;
        }
